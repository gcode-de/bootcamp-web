...
const crazySlider = document.querySelector('[data-js="input-crazy"]');
...
crazySlider.addEventListener("input", () => {
  box.style.transform = `rotate(${crazySlider.value}deg)`;
  box.style.backgroundColor = `hsl(${crazySlider.value}, 70%, 60%)`;
  box.style.borderRadius = `${(crazySlider.value / 360) * 50}%`;
});
