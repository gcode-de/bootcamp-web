...
function App() {
  ...
  function handleToggleFavorite(id) {
    setEntries((entries) => entries.map((entry) => (entry.id === id ? { ...entry, isFavorite: !entry.isFavorite } : entry)));
  }
  // Inside this function update the state by calling `setEntries`.
  // To create a new state value iterate over the `entries` array and search for the entry matching the `id`.
  // If the `id` matches invert the value of `isFavorite`. (hint: you can use the array method `map` for this)
